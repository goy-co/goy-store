...
async fn test_local_blob_store_operations() {
    ...
    // 1. Put
    store.blob.put(key, &payload, Some(meta.clone())).await.unwrap();
    ...
    assert_eq!(fetched_meta.content_type, Some("application/json".to_string()));
    ...
    // 4. Presign URL
    let url = store.blob.presign_url(key, Duration::from_secs(60)).await.unwrap();
